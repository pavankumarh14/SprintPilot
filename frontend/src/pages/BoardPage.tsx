...
export default function BoardPage() {
  ...
  async function persistColumnChange(ticketId: string, newColumn: Column): Promise<void> {
    try {
      ...
      console.error("[Board] Failed to persist column change:", err);
    }
  }
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={{ color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: 12 }}>
            Sprint 9 &middot; Drag tickets between columns
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div style={styles.colHeader}>
                ...
                <div style={styles.colCount}>{tickets.length}</div>
              </div>
    ...
    >
      ...
      <div style={cardStyles.labels}>
        {(ticket.labels ?? []).slice(0, 3).map((l: string) => (
          <span key={l} className={LABEL_COLORS[l] ?? "badge-gray"}>{l}</span>
        ))}
      </div>
    </div>
  );
